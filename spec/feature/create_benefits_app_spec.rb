...
RSpec.feature 'a user creating a new benefits application' do
  ...
  def add_primary_member(submit = false, check = false)
    ...
    click_on 'Add Member →'
    click_on 'Continue' if submit
    expect(page).to have_text 'Tell us about your secondary member' if check
  end

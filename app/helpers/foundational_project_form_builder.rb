class FoundationalProjectFormBuilder < Cfa::Styleguide::CfaFormBuilder
  ...
  def h2_label_contents(label_text, help_text, optional = false)
    label_text = <<~HTML
      <h2 class="form-question">#{label_text + optional_text(optional)}</h1>
    HTML
  ...
  def vita_min_text_field(
    ...
    text_field_options = standard_options.merge(
      class: (classes + ["text-input form-width--med"]).join(" "),
      ).merge(options).merge(error_attributes(method: method))
    ...
    formatted_label = label(method, h2_label_contents(label_text, help_text), options)
